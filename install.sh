#!/usr/bin/env sh
# Stolen from:
# https://github.com/librephoenix/nixos-config/blob/main/install.sh

# Automated script to install dotfiles
if [ $# -gt 0 ]
  then
    SCRIPT_DIR=$1
  else
...
# Clone dotfiles
nix-shell -p git vim --command "git clone https://github.com/dimitri-lopez/nixos-config $SCRIPT_DIR"
...
echo "See $SCRIPT_DIR/install.org for next steps"
