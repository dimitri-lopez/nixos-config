#+title: PhD / Academic Tools

* PhD / Academic Tools

** phd.nix
:PROPERTIES:
:header-args:nix: :noweb-ref phd.nix
:END:
#+begin_src nix :tangle ./modules/phd.nix :noweb yes
{ config, lib, pkgs, ... }:
...
{
  <<phd.nix-packages>>
  <<phd.nix-sessionVariables>>
  <<phd.nix-desktopEntries>>
}
#+end_src

*** GLEAMviz Installer Script
#+begin_src nix :noweb-ref gleamviz-installer
...
*** Packages
#+begin_src nix :noweb-ref phd.nix-packages
...
*** Session Variables
#+begin_src nix :noweb-ref phd.nix-sessionVariables
  # arbovirus cache
...
*** Desktop Entries
#+begin_src nix :noweb-ref phd.nix-desktopEntries
