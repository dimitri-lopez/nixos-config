...
in
{
  ...
  home.sessionVariables = {
    QT_QPA_PLATFORM_PLUGIN_PATH = "${unstable.qt6.qtbase}/lib/qt-6/plugins";
  };
  ...
  home.packages = with pkgs; [
    ...
    xdotool
    wmctrl # EAF window-focus helper (eaf.el calls wmctrl -m / wmctrl -i -a)
    xdg-utils # xdg-open used by eaf-open-external
    xclip
    # EAF python env; eaf-python-command points at this wrapper
    (pkgs.writeScriptBin "eaf-python" ''
      #!/usr/bin/env bash
      exec ${(unstable.python313.withPackages (ps: with ps; [ pyqt6 pyqt6-sip pyqt6-webengine epc sexpdata tld lxml pymupdf packaging ]))}/bin/python3 "$@"
    '')
    unzip # for dired
    ...
    ((emacsPackagesFor emacs30-gtk3).emacsWithPackages (epkgs: [
      ...
      (epkgs.trivialBuild {
        ...
        buildInputs = [ pkgs.gcc pkgs.gnumake pkgs.pkg-config unstable.mupdf ];
        # build the dynamic module + autoloads (make emits render-core.so and
        # reader-autoloads.el); trivialBuild's byte-compile then runs with
        # `-L .` so the cross-file (require 'reader) / (require 'render-core)
        # resolve, unlike melpaBuild's elpa2nix recompile.
        preBuild = "make clean all";
        # trivialBuild installs only *.el/*.elc. Install the .so and autoloads
        # here (in installPhase, BEFORE the generic native-comp postInstall)
        # so native-compilation can resolve (require 'render-core).
        installPhase = ''
          runHook preInstall
          LISPDIR=$out/share/emacs/site-lisp
          install -d $LISPDIR
          install *.el *.elc $LISPDIR
          install -m755 render-core.so $LISPDIR/
          install reader-autoloads.el $LISPDIR/
          runHook postInstall
        '';
      })
    ]))
    ...
    libertinus
    libre-baskerville
    unstable.literata  # not in nixos-25.05, only unstable
