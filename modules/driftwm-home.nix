...
{
  home.activation.checkDriftwmConfig = ''
    ${pkgs.writeShellScript "driftwm-check-config" "exec ${pkgs.driftwm or pkgs.bash}/bin/bash -c 'echo driftwm config check skipped (no check-config command yet)'"} || true
  '';
  ...
  home.file = {
    ".config/driftwm/config.toml".text = ''
      ...
      # Copy of defaults with personal overrides
